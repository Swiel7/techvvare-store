...
export const redirectToUrl = async (url: string, type?: RedirectType): Promise<void> => {
  try {
    redirect(url, type);
  } catch (error) {
...
export const loginWithCredentials = async (values: z.infer<typeof loginSchema>): Promise<TActionResult> => {
  try {
    ...
    const message = error instanceof CredentialsSignin ? 'Invalid email or password!' : 'An unexpected error occurred!';

    return { success: false, message };
  }
};

export const register = async (values: z.infer<typeof registerSchema>): Promise<TActionResult> => {
  try {
    ...
    const createUserResult = await createUser({ firstName, lastName, email, password: hashedPassword });

    if (!createUserResult.success) {
      return { success: false, message: createUserResult.message || 'Failed to create user account!' };
    }

    const loginResult = await loginWithCredentials({ email, password });

    if (!loginResult.success) {
      return { success: false, message: `Account created, but auto-login failed: ${loginResult.message}` };
    }
...
export const logout = async (options?: { redirectTo?: string; redirect?: true | undefined }): Promise<void> => {
  try {
